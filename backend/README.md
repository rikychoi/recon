...
### 서브도메인 열거의 와일드카드/하이재킹 자동 제외

하이재킹 환경에서는 서브도메인 브루트포스가 **존재하지 않는 이름까지 전부 "해석됨"으로
착각**해 가짜 서브도메인 수십 개가 잡히는 노이즈가 생깁니다. `recon`은 이를 자동으로 거릅니다:

- 열거 전에 **무작위 이름**(`recon-<hex>.<도메인>`)을 먼저 조회
- 그게 해석되면 그 응답 IP를 **와일드카드/하이재킹 IP로 기록**
- 브루트포스 결과 중 **그 IP로만 해석되는 후보는 가짜로 보고 제외**(다른 IP를 가진 실제 서브도메인은 유지)

감지 시 진행 로그에 `와일드카드 DNS(하이재킹) 감지: N개 IP …` 가 출력됩니다.
따라서 DNS를 `1.1.1.1`/`8.8.8.8`로 바꾸지 않아도 국내 ISP 환경에서 노이즈 없이 동작합니다.
