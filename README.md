...
3. **포트 스캔** — 각 IP에서 열린 포트와 서비스를 확인합니다.
4. **취약점 점검** — 열린 포트를 대상으로 `nuclei`(템플릿 기반 점검)와
   `metasploit`(실제 CVE 점검)을 돌려 취약점을 찾고, CVE·CVSS·심각도를 정리합니다.
